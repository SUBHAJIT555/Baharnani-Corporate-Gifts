...
const ProductDetails = () => {
  ...
  const fetchSeo = async () => {
    if (product) {
      return {
        ...
        og_title: resolveValue(
          product.seo.og_title,
          product.seo.title,
          product.name + " | Baharnani Gifts"
        ),
        og_desc: resolveValue(
          product.seo.og_desc,
          product.seo.description,
          cleanMetaDescription(product.short_desc)
        ),
  ...
  const galleryImages = useMemo(() => {
    ...
    const images = [product.main_image, ...product.gallery].map(
      (url, index) => ({
        original: url,
        thumbnail: url,
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
          ...
          <div className="space-y-6">
            ...
            <button
              ...
              className={cn(
                "group relative w-full font-tanker font-medium py-4 px-6 rounded-md transition-colors duration-200 text-lg overflow-hidden",
